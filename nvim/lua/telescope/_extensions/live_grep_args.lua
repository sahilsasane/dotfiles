-- Local override for telescope-live-grep-args until upstream stops calling
-- deprecated `vim.tbl_flatten` on Neovim 0.11+.
...
local function flatten(parts)
  if vim.fn.has 'nvim-0.11' == 1 then return vim.iter(parts):flatten(math.huge):totable() end
  ---@diagnostic disable-next-line: deprecated
  return vim.tbl_flatten(parts)
end
...
local function live_grep_args(opts)
  ...
  local function cmd_generator(prompt)
    ...
    return flatten { args, prompt_parts, opts.search_dirs }
  end
