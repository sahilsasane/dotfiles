...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Catppuccin Mocha: blue tail into mauve near the current cursor.
    const vec4 TRAIL_TAIL_COLOR = vec4(0.537, 0.706, 0.980, 1.0);
    const vec4 TRAIL_HEAD_COLOR = vec4(0.796, 0.651, 0.969, 1.0);
    vec4 trailColor = mix(TRAIL_TAIL_COLOR, TRAIL_HEAD_COLOR, fadeFactor);
